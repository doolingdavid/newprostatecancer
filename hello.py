...
class NameForm(Form):
    ...
    laterality = SelectField('Laterality', default='0',
        choices = [('involvement','Bilateral involvement, lateral origin unknown; stated to be single primary'),
    ...
    radiation = SelectField('rx_summ_radiation', default='0',
        choices = [('beam', 'Beam radiation'),
                   ('combination', 'Combination of Beam and either implants or isotopes'),
                   ('none', 'None; diagnosed at autopsy'),
                   ('refused',"Patient or patient's guardian refused radiation therapy"),
                   ('implants','Radioactive implants'),
                   ('nos', 'Radiation, NOS - method or source not specified'),
                   ('radioisotopes','Radioisotopes'),
                   ('recommended','Radiation recommended, unknown if administered'),
                   ('unknown','Unknown if radiation administered')],
            validators=[Required()])
    ...
    seerhistoric = SelectField('seer_historic_stage_a', default='0',
        choices = [('distant','Distant'),
                   ('in','In situ'),
                   ('localizedregional','Localized/Regional'),
                   ('unstaged','Unstaged')],
            validators=[Required()])
...
clf = joblib.load('MALEGENCLASSIFIER/rfmalegen.pkl')
...
def results():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if form.laterality.data == 'involvement':
            session['laterality_involvement'] = '1'
        else:
            session['laterality_involvement'] = '0'
        ...
        if form.radiation.data == 'beam':
            session['radiation_beam'] = '1'
        else:
            session['radiation_beam'] = '0'


        if form.radiation.data == 'combination':
            session['radiation_combination'] = '1'
        else:
            session['radiation_combination'] = '0'


        if form.radiation.data == 'none':
            session['radiation_none'] = '1'
        else:
            session['radiation_none'] = '0'
            
        if form.radiation.data == 'refused':
            session['radiation_refused'] = '1'
        else:
            session['radiation_refused'] = '0'

        if form.radiation.data == 'implants':
            session['radiation_implants'] = '1'
        else:
            session['radiation_implants'] = '0'

        if form.radiation.data == 'nos':
            session['radiation_nos'] = '1'
        else:
            session['radiation_nos'] = '0'


        if form.radiation.data == 'radioisotopes':
            session['radiation_radioisotopes'] = '1'
        else:
            session['radiation_radioisotopes'] = '0'

        if form.radiation.data == 'recommended':
            session['radiation_recommended'] = '1'
        else:
            session['radiation_recommended'] = '0'

        if form.radiation.data == 'unknown':
            session['radiation_unknown'] = '1'
        else:
            session['radiation_unknown'] = '0'
        ...
        if form.seerhistoric.data == 'localizedregional':
            session['seerhistoric_localizedregional'] = '1'
        else:
            session['seerhistoric_localizedregional'] = '0'
        ...
        session_data = np.array( [session['maritalstatus_divorced'],
                                  ...
                                  session['grade_un'],
                                  session['grade_we'],
                                  session['laterality_involvement'],
                                  ...
                                  session['seerhistoric_localizedregional'],
                                  session['seerhistoric_unstaged'],
                                  session['radiation_beam'],
                                  session['radiation_combination'],
                                  session['radiation_none'],
                                  session['radiation_refused'],
                                  session['radiation_implants'],
                                  session['radiation_nos'],
                                  session['radiation_radioisotopes'],
                                  session['radiation_recommended'],
                                  session['radiation_unknown'],
                                  session['hist_acinar'],
                                  ...
                                  session['yod'],
                                  session['cs_tumor_size'],
                                  session['elevation'],
                                  session['lat'],
                                  session['lng']]).astype('float')
